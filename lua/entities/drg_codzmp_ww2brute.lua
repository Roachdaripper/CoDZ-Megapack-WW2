...
ENT.Models = {"models/roach/codz_megapack/ww2/brute.mdl"}
...
ENT.PossessionBinds = {
	[IN_ATTACK] = {{coroutine = true,onkeydown = function(self)
		if not self:IsRunning() then
			...
			self:PlaySequenceAndMove("att_run",1,self.PossessionFaceForward)
		end
	end}},
...
function ENT:CustomInitialize()
	...
	self.SOUND_EVTTABLE = {
		...
		["s2_zom_brt_walk_attack_v2"] = {"codz_megapack/ww2/brute/vox/s2_zom_brt_walk_attack_v2_0", 4},
	...
	self:Timer(0.1,self.SetCollisionBounds, Vector(-100,-100,0), Vector(100, 100, 220))
...
function ENT:OnMeleeAttack(enemy)
	if self.Charging then
		self.Charging = false
		self:PlaySequenceAndMove("att_run",1,self.FaceEnemy)
	else
		self:PlaySequenceAndMove("att"..math.random(5),1,self.FaceEnemy)
	end
end
...
function ENT:OnTakeDamage(dmg,hg)
	...
	if dmg:IsDamageType(DMG_BLAST) or ((dmg:GetDamage()>=40) and (math.random(50)<=12)) then
		...
		self.Charging = false
		self:CICO(function()
			self:PlaySequenceAndMove("att_stagger")
